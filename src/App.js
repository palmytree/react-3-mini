...
class App extends Component {
	...
	// 	// 		toast.success('Yo! I got your vehicles to display');
	// 	// 	})
	// 	// 	.catch(err => {
	// 	// 		console.log(err);
	// 	// 		toast.error(`Uh oh! We couldn't find your vehicles`);
	// 	// 	});
	// }
	...
	async getVehicles() {
		// axios (GET)
		// setState with response -> vehiclesToDisplay
    const res = await axios.get( 'https://joes-autos.herokuapp.com/api/vehicles' ).catch( err => toast.error( 'Failed to get vehicles' ) )
    this.setState( {
	...
	sellCar(id) {
		...
		axios
			...
			.catch(err => toast.error(`Whoops! Couldn't delete.`));
	}
	...
	addCar() {
		...
		axios
			...
			.then(res => {
				this.setState({ vehiclesToDisplay: res.data.vehicles });
				toast.success(
					`You did it! Added a ${newCar.year} ${newCar.make} ${newCar.model}`
				);
			})
			.catch(err => toast.error('Whoops! Car not added'));
	}
